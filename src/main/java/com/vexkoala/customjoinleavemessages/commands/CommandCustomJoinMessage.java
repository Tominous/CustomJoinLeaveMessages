...
public class CommandCustomJoinMessage implements TabExecutor
{
            ...
            if (args.length > 0)
            {
                ...
                if (args[0].equalsIgnoreCase("user")) // /cjm user [username] set [message]
                {
                    ...
                    if (args[2].equalsIgnoreCase("set"))
                    {
                        ...
                        // Get max characters
                        int maxChar = plugin.getConfig().getInt("messages.join.max-characters");
                ...
                else if (args[0].equalsIgnoreCase("set")) // /cjm set [message]
                {
    ...
    private static String formatMessage(String string, Player player, String message)
    {
        ...
        if (player.hasPermission("cjlm.customjoinmessage.color")) // If player has permission
        {
            // Format [message]
            string = string.replaceAll("\\[message\\]", message);
            // Format colors
